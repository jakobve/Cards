package es.uam.eps.dadm.cards.ui.adapter
...
class DeckAdapter: RecyclerView.Adapter<DeckAdapter.DeckHolder>() {
    ...
    inner class DeckHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        fun bind(deck: Deck) {
            ...
            binding.editDeckButton.setOnClickListener {
                it.findNavController()
                    .navigate(
                        DeckListFragmentDirections.actionDeckListFragmentToDeckEditFragment(
                            deck.deckId
                        )
                    )
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewTye: Int): DeckHolder {
